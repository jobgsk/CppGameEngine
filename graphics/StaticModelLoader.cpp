...
StaticModelLoader::StaticModelLoader(const char* path, const char *mpath)
{
	m_infile.open(mpath);
	if (!m_infile) {
...
/*void StaticModelLoader::load_texture(const char * dir_name)
{
	...
	for (int i = 0; i < mat_num; i++) {
		if (mat_array[i].map_kd.size() > 0) {
			...
			GLuint texture = Graphics::LoadTexture(path);
			//GLuint texture = Graphics::LoadTgaTexture(path);
...
*/
ModelsStruct *CopyModel(StaticModelLoader* sml, float scale)
{
	...
	{
		model->v_array[i].ox = v_temp[i].ox * scale, model->v_array[i].x = v_temp[i].x * scale;
		model->v_array[i].oy = v_temp[i].oy * scale, model->v_array[i].y = v_temp[i].y * scale;
		model->v_array[i].oz = v_temp[i].oz * scale, model->v_array[i].z = v_temp[i].z * scale;
		model->v_array[i].ow = v_temp[i].ow * scale, model->v_array[i].w = v_temp[i].w * scale;
...
{
	return CopyModel(sml, 1);
}


ModernModelsStruct *ModernCopyModel(StaticModelLoader* sml, float scale, glm::vec4 color)
{
	ModernModelsStruct *model = new ModernModelsStruct;
	//model->v_num = sml->get_v_num();
	//model->vt_num = sml->get_vt_num();
	//model->vn_num = sml->get_vn_num();
	...
	//model->m_array = (mat_struct*)calloc(model->m_num, sizeof(mat_struct));
	model->m_num = sml->get_mat_num();
	model->m_array = new mat_struct[model->m_num];
	

	int test1 = 0;
	int f_num = 0, m_index = 0, m_num = 0;
	for (int i = 0; i < sml->get_f_num(); i++)
	{
		if (m_num == 0 || m_index != f_temp[i].material->index)
		{
			m_index = f_temp[i].material->index;
			model->m_array[m_num].start_geometry_index = f_num;
			//previouse end index
			if (m_num > 0)
				model->m_array[m_num - 1].end_geometry_index = f_num;

			/*
			material data
			*/
			model->m_array[m_num].newmtl = f_temp[i].material->newmtl;

			model->m_array[m_num].kar = f_temp[i].material->kar;
			model->m_array[m_num].kag = f_temp[i].material->kag;
			model->m_array[m_num].kab = f_temp[i].material->kab;

			model->m_array[m_num].kdr = f_temp[i].material->kdr;
			model->m_array[m_num].kdg = f_temp[i].material->kdg;
			model->m_array[m_num].kdb = f_temp[i].material->kdb;

			model->m_array[m_num].ksr = f_temp[i].material->ksr;
			model->m_array[m_num].ksg = f_temp[i].material->ksg;
			model->m_array[m_num].ksb = f_temp[i].material->ksb;

			model->m_array[m_num].illum = f_temp[i].material->illum;
			model->m_array[m_num].ns = f_temp[i].material->ns;
			model->m_array[m_num].d = f_temp[i].material->d;
			model->m_array[m_num].map_kd = f_temp[i].material->map_kd;
			model->m_array[m_num].index = m_index;
	...
	for (int i = 0; i < m_num; i++)
	{
		std::cout << "  mtest " 
			<< i
			<< " " << model->m_array[i].index
			<< " " << model->m_array[i].start_geometry_index
			<< " " << model->m_array[i].end_geometry_index
			<< std::endl;
	}

	

	model->v_array = new float[3 * model->f_num];
	model->vt_array = new float[2 * model->f_num];
	model->vn_array = new float[3 * model->f_num];
	model->c_array = new float[4 * model->f_num];
	...
	for (int i = 0; i < sml->get_f_num(); i++)
	{
		for (int j = 0; j < f_temp[i].fi_num; j++)
		{
			/*
				verticies
			*/
			model->v_array[(f_num * 3) + 0] = f_temp[i].fi_array[j].v->x * scale;
			model->v_array[(f_num * 3) + 1] = f_temp[i].fi_array[j].v->y * scale;
			model->v_array[(f_num * 3) + 2] = f_temp[i].fi_array[j].v->z * scale;
			/*
				textures
			*/
			model->vt_array[(f_num * 2) + 0] = f_temp[i].fi_array[j].vt->u;
			model->vt_array[(f_num * 2) + 1] = f_temp[i].fi_array[j].vt->v;
			/*
				normals
			*/
			model->vn_array[(f_num * 3) + 0] = f_temp[i].fi_array[j].vn->x;
			model->vn_array[(f_num * 3) + 1] = f_temp[i].fi_array[j].vn->y;
			model->vn_array[(f_num * 3) + 2] = f_temp[i].fi_array[j].vn->z;

			/*
				color
			*/
			model->c_array[(f_num * 4) + 0] = color.r;
			model->c_array[(f_num * 4) + 1] = color.g;
			model->c_array[(f_num * 4) + 2] = color.b;
			model->c_array[(f_num * 4) + 4] = color.a;
	...
	model->rx = sml->get_rx() * scale;
	model->ry = sml->get_ry() * scale;
	model->rz = sml->get_rz() * scale;

	model->loader = sml;
	return model;
};

ModernModelsStruct *ModernCopyModel(StaticModelLoader* sml, float scale)
{
	return ModernCopyModel(sml, scale, glm::vec4(1, 1, 1, 1));
};

ModernModelsStruct *ModernCopyModel(StaticModelLoader* sml)
{
	return ModernCopyModel(sml, 1, glm::vec4(1, 1, 1, 1));
};
